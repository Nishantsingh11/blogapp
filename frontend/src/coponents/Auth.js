...
const Auth = () => {
 ...
 const res = await axios.post(`http://localhost:5000/api/user/${type}`,{
    ...
    const handleSubmit = (e) =>{
      ...
      console.log(inputs);
    if(isSignup){
      sendRequest("signup").then((data)=>localStorage.setItem("userId",data.user._id)).then(()=>dispath(authAction.login())).then(()=>negivate("/blogs")).then(data=>console.log(data))
    }
    else{
      sendRequest().then((data)=>localStorage.setItem("userId",data.user._id)).then(()=>dispath( authAction.login())).then(()=>negivate("/blogs")).then(data=>console.log(data))
    }
    }
  return (
    <div>
